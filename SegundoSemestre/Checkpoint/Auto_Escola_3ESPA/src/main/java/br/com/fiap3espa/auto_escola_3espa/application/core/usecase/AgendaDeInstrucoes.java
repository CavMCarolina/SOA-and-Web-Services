...
public class AgendaDeInstrucoes {
    ...
    public DadosDetalhamentoInstrucao agendar(@Valid DadosAgendamentoInstrucao dados) {
        ...
        Instrucao instrucao = new Instrucao(null, aluno, instrutor, dados.data(), false, null);
    ...
    public void cancelar(@Valid DadosCancelamentoInstrucao dados) {
        ...
        if (Boolean.TRUE.equals(instrucao.getCancelada())) {
            throw new ValidacaoException("Esta instrução já foi cancelada!");
        }

        long horasAteInstrucao = Duration.between(LocalDateTime.now(), instrucao.getData()).toHours();
        if (horasAteInstrucao < 24) {
